...
class AABB(object):  # pylint: disable=useless-object-inheritance
    ...
    'Here I start adding my bits of code'
    def draw(self,WIN,color):
        width = self.limits[0][1]-self.limits[0][0]
        height = self.limits[1][1]-self.limits[1][0]
        pygame.draw.rect(WIN, color, pygame.Rect(self.limits[0][0], self.limits[1][0], width,height),1)
...
class AABBTree(object):  # pylint: disable=useless-object-inheritance
    ...
    def draw(self,WIN,color):
        if self.aabb!=AABB():
            self.aabb.draw(WIN,color)

        if self.left!=None:
            self.left.draw(WIN,color)

        if self.right!=None:
            self.right.draw(WIN,color)
